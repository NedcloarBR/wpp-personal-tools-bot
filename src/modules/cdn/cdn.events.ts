...
export class CdnEvents {
	...
	public async onCDNMessage(@Context() [message]: ContextOf<Events.MESSAGE_CREATE>): Promise<void> {
		const chatId = message.id.remote;
		this.logger.log(`[onCDNMessage] chatId=${chatId} fromMe=${message.fromMe} hasMedia=${message.hasMedia} body="${message.body}"`);
		...
		const results = await this.cdnService.flushByTrigger(chatId, message.body.trim());
		for (const { name, saved } of results) {
			if (saved > 0) await message.reply(`✅ *${name}*: ${saved} arquivo(s) salvos!`);
		}
	}
	...
	private async handleMedia(message: Message, chatId: string): Promise<void> {
		try {
			...
			if (!media) {
				this.logger.warn(`[handleMedia] downloadMedia() returned null chatId=${chatId}`);
				return;
			}
			this.logger.log(`[handleMedia] downloaded mimetype=${media.mimetype} filename=${media.filename}`);
