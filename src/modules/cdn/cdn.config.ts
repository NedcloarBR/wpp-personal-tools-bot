...
function defaultSavePath(): string {
	const wslUser = process.env.WSLENV && process.env.USERPROFILE
		? process.env.USERPROFILE.replace(/\\/g, "/").replace(/^([A-Za-z]):/, (_, d) => `/mnt/${d.toLowerCase()}`)
		: null;
...
export const cdnConfig = registerAs("cdn", () => ({
	...
	configsFile: process.env.CDN_CONFIGS_FILE ?? path.join(os.homedir(), ".cdn-configs.json"),
